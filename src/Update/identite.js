...
export default class identite extends Component{
    ...
    handleDelete = () => {
        ...
        db.ref('users/' + userKey).update({identite: ""}).then(r  =>{this.getUserKey();alert("Image deleted")});
    };
        ...
        db.ref("users").orderByChild("uid").equalTo(uid)
            .once("value",function (snapshot) {
                snapshot.forEach(function (child) {
                    let Data= child.val();
                    Change(child.key);
                    getImage(Data.identite);
                })
            });
        const getImage= async (dataImage) => {
            if (dataImage === "" || dataImage === undefined || dataImage === null){
                let ref = storage.ref('IdentiteImage/identite.png');
                let url = await ref.getDownloadURL();
                this.setState({identiteImage:url})
            }else {
                let ref = storage.ref('IdentiteImage/'+uid+'/'+dataImage);
                let url = await ref.getDownloadURL();
                this.setState({identiteImage:url})
            }
        };
    ...
    uploadImage = async (uri) => {
        ...
        let ref = storage.ref().child("IdentiteImage/"+uid+"/"+imageNumber);
        db.ref('users/'+userKey).update({identite:imageNumber});
        return ref.put(blob);
    };
    render() {
        let uri =this.state.identiteImage;
        return (
            <View style={styles.container}>
                <Text style={styles.text1 }> Votre Idendite !!</Text>
